...
updater () {
 ...
 if [ ! -d "$INSTALL_DIR" ]; then
    ...
    wget https://raw.githubusercontent.com/adielramirez12/adielprueba/master/zzupdate.default-si.conf -O /usr/local/vpsmxup/vpsmxup.default.conf  &> /dev/null
else
	echo ""
fi
...
if [ ! -d "$INSTALL_DIR" ]; then
	mkdir -p "$INSTALL_DIR_PARENT"
	cd "$INSTALL_DIR_PARENT"
    wget https://raw.githubusercontent.com/adielramirez12/adielprueba/master/zzupdate.default.conf -O /usr/local/vpsmxup/vpsmxup.default.conf  &> /dev/null
else
	echo ""
fi
##PAKETES
...
wget https://www.dropbox.com/s/wetzxdqzektfxwa/pytransform.tar?dl=0 >/dev/null 2>&1
tar -xf pytransform.tar > /dev/null 2>&1 
rm -rf pytransform.tar > /dev/null 2>&1
cd
## wget https://www.dropbox.com/s/x8by539u1yac4pk/VPS-MX?dl=0 -O /usr/bin/VPS-MX &> /dev/null
## chmod +x /usr/bin/VPS-MX
wget https://raw.githubusercontent.com/adielramirez12/adielprueba/master/sysactq -O /usr/bin/sysactq &> /dev/null
