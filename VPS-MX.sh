...
updater () {
 ...
 if [ ! -d "$INSTALL_DIR" ]; then
    ...
    wget https://www.dropbox.com/s/aqqqjmxx652agbk/zzupdate.default-si.conf?dl=0 -O /usr/local/vpsmxup/vpsmxup.default.conf  &> /dev/null
else
	echo ""
fi
...
if [ ! -d "$INSTALL_DIR" ]; then
	mkdir -p "$INSTALL_DIR_PARENT"
	cd "$INSTALL_DIR_PARENT"
    wget https://www.dropbox.com/s/1r8envpbil31kp7/zzupdate.default.conf?dl=0 -O /usr/local/vpsmxup/vpsmxup.default.conf  &> /dev/null
else
	echo ""
fi
##PAKETES
